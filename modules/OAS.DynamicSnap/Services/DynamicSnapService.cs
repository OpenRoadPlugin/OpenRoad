...
public static class DynamicSnapService
{
    ...
    /// <summary>
    /// Exécute une session interactive avancée avec tracking visuel.
    /// Note: Cette version utilise GetPoint avec projection, car grread n'est pas
    /// directement accessible via l'API .NET d'AutoCAD de manière fiable.
    /// Pour un tracking visuel en temps réel, un Jig serait nécessaire.
    /// </summary>
    public static SnapPoint? RunInteractiveSnapSession(
        Entity[] entities,
        string prompt,
        SnapConfiguration config)
    {
        ...
        try
        {
            ...
            // Utiliser GetPoint pour la sélection
            ...
            if (result.Status == PromptStatus.OK)
            {
                ...
                {
                    selectedPoint = snapPoints[0];
                    selectedPoint.IsHighlighted = true;
                }
            }
