/*
 * ??????????????????????????????????????????????????????????????????????????????
 * TEMPLATE DE MODULE OPEN ROAD
 * ??????????????????????????????????????????????????????????????????????????????
 * 
 * Ce fichier sert de modèle pour créer un nouveau module Open Road.
 * 
 * INSTRUCTIONS:
 * 1. Créez un nouveau projet avec le nom OpenRoad.{VotreModule}
 * 2. Copiez ce fichier et CommandTemplate.cs dans votre projet
 * 3. Copiez OpenRoad.Module.Template.csproj et adaptez-le
 ...
 * 
 * RAPPEL: Le nom de l'assembly DOIT commencer par "OpenRoad." pour être détecté.
 * 
...
public class MonModuleModule : ModuleBase
{
    ...
    /// Format: "pack://application:,,,/OpenRoad.MonModule;component/Resources/icon.png"
    /// </summary>
